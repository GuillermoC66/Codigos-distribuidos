...
int main (int argc, char** argv){	
	...
	{ 
		int mensaje=42; 
	 	...
	 	printf("Proceso %d envia mensaje %d  al proceso 1 \n", rank, mensaje); 
	 	MPI_Send(&mensaje, 1, MPI_INT, 1, 0, MPI_COMM_WORLD);
	 	MPI_Recv(&mensaje,1,MPI_INT,1,0, MPI_COMM_WORLD, &status); 
	 	printf("Proceso %d  recibio %d al proceso 1\n", rank, mensaje );
    }
    if (rank ==1)
	{ 
		int mensaje1; 
        MPI_Recv(&mensaje1, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, &status); 
		printf("Proceso %d recibio  %d al proceso 0 \n", rank, mensaje1);
		mensaje1+=4; 
		MPI_Send(&mensaje1, 1, MPI_INT, 0, 0, MPI_COMM_WORLD);
    }
    MPI_Finalize(); 
	return 0;
}
